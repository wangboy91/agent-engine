...
from app.application.execution.skill_runtime import SkillRuntime, SkillRuntimeError
...
from app.application.tool.executor import ToolExecutor as CanonicalToolExecutor
from app.domain.agent import AgentResponse, AgentTurnState, RouteDecision, SceneMapping
...
from app.infrastructure.persistence.secret_store import InMemorySecretStore
...
from app.infrastructure.tool_runners.api_tool import ApiToolRunner
...
def test_framework_exposes_ports_and_explicit_states(tmp_path: Path) -> None:
    ...
    assert app.infrastructure.__doc__
    assert app.interfaces.__doc__
...
def test_agent_runtime_uses_canonical_ddd_import_paths() -> None:
    assert AgentLoop.__module__ == "app.application.agent.state_machine"
    assert SkillRouter.__module__ == "app.application.agent.router"
    assert InputResolver.__module__ == "app.application.agent.input_resolver"
    assert AgentResponse.__module__ == "app.domain.agent.schemas"
    assert RouteDecision.__module__ == "app.domain.agent.schemas"
    assert SceneMapping.__module__ == "app.domain.agent.scene_mapping"


def test_skill_and_tool_runtime_use_canonical_ddd_import_paths() -> None:
    assert SkillRuntime.__module__ == "app.application.execution.skill_runtime"
    assert CanonicalToolExecutor.__module__ == "app.application.tool.executor"
    ...
    assert load_skill.__module__ == "app.infrastructure.package_loaders.skill_loader"
    assert load_tool.__module__ == "app.infrastructure.package_loaders.tool_loader"

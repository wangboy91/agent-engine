# BKL Skill Engine 架构总览与演进

> 本文以当前代码为准，描述 0.1.0 工程验证版的真实边界，并给出持续迭代时应保持的架构约束。

## 1. 架构目标

BKL 要解决的是“将业务 AI 能力做成可安装、可调用、可观察、可治理的运行单元”。核心原则：
...
| 目录 | 主要职责 | 不应承担的职责 |
| --- | --- | --- |
| `bkl_engine/domain/` | Pydantic 领域模型、状态、错误、纯规则对象 | 文件 I/O、HTTP、子进程、框架对象 |
| `bkl_engine/application/` | 用例、运行时编排、Ports、Policy、Agent 状态机 | provider/数据库具体实现 |
| `bkl_engine/infrastructure/` | 配置、包加载、模型 provider、Tool runner、持久化适配器 | 把业务流程写死到基础设施中 |
| `bkl_engine/interfaces/` | Typer/FastAPI、请求转命令、响应/流式输出 | 绕过 Engine 调用 Runtime |
| `resources/` | 可安装的 Skill/Tool/示例输入 | 引擎内部代码 |
| `tests/` | 以行为验证分层边界和运行契约 | 依赖本机真实密钥或外部服务 |

`tests/test_architecture_layers.py` 已对部分层间依赖做约束。新增模块时，优先增加 Port 和适配器，而不是从应用层直接 import 具体 JSON/HTTP 实现。
...
Workflow 在 `bkl.skill.json` 中声明子 Skill 和 `depends_on`。无依赖图时顺序执行；存在依赖时使用 DAG 调度，最多并发 `max_parallel_steps` 个就绪节点。父 Run 聚合子步骤输出、用量、trace 摘要和 artifacts。子 Run 始终独立持久化，便于追查和重跑。
...
| 数据 | 当前实现 | 适用范围 | 生产目标 |
| --- | --- | --- | --- |
| Catalog | `.bkl/catalog.json` | 单实例资源路径缓存 | 包版本仓库 + 数据库索引 |
| Run/Trace/Session/Workspace/Policy | `.bkl/*.json` | 本地、单进程演示 | PostgreSQL，按 workspace 分区/索引 |
| Secret | `.bkl/secrets.json` 明文 | 开发调试 | KMS/Vault，审计与轮换 |

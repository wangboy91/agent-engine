...
状态：本迭代架构基线  
关联：[VERSION_1.0.1](VERSION_1.0.1.md) · [ADR 租户层级](ADR_企业智能体租户与Workspace层级设计.md) · [账号与业务系统接入](账号与业务系统接入.md)

## 1. 一句话架构

**一个执行内核（SkillEngine），两层平台模型（平台模板 / 租户实例），两个产品入口；运行数据以 Tenant 为硬隔离根；账号登录由外部账号权限系统/业务 IAM 提供，基座只消费 Principal。**

```text
┌─────────────────────────────────────────────────────────────────┐
│  账号权限系统 / 业务 IAM（独立可部署，基座不管员工账号）               │
└────────────────────────────┬────────────────────────────────────┘
                             │ JWT / 可信头 → Principal
┌────────────────────────────▼────────────────────────────────────┐
│  产品入口                                                         │

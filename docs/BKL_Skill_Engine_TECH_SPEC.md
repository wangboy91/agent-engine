...
5. 添加测试用例。
6. 在 engine/resources/tools/subtitle_generate_srt 下创建一个示例 Tool。
```

验收标准：

```text
可以成功加载 engine/resources/tools/subtitle_generate_srt/tool.yaml
缺少必填字段时会报清晰错误
pytest 通过
```
...
registries:
  skills_dir: ./engine/resources/skills
  tools_dir: ./engine/resources/tools
